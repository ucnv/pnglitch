...
module PNGlitch
  ...
  class Filter
    ...
    def self.guess(filter_type)
      type = nil
      type = if filter_type.is_a?(Numeric) && filter_type.between?(NONE, PAETH)
               filter_type.to_i
             elsif filter_type.is_a?(String) && filter_type =~ /^[0-4]$/
               filter_type.to_i
             else
               @@types.collect { |c| c.to_s[0] }.index(filter_type.to_s[0].downcase)
             end
      type
    end
    ...
    def initialize(filter_type, sample_size)
      ...
      @encoder = method ('encode_%s' % @filter_type_name.to_s).to_sym
      @decoder = method ('decode_%s' % @filter_type_name.to_s).to_sym
    end
    ...
    def encode_none(data, _prev) # :nodoc:
      data
    end

    def decode_none(data, _prev) # :nodoc:
      data
    end

    def encode_sub(data, _prev) # :nodoc:
    ...
    def decode_sub(data, _prev) # :nodoc:
